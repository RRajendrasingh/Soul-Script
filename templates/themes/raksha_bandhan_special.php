<?php
// templates/themes/raksha_bandhan_special.php
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php foreach ($promisesList as $pIdx => $pText): ?>
      <div class="bg-[#221f21] border border-[#4d444b] rounded-2xl p-6 space-y-4 hover:border-[#eac34a]/60 transition-all shadow-xl relative group">
        ...
        <div class="text-[10px] text-[#eac34a] uppercase tracking-wider font-semibold">Promise to <?= $partnerName ?></div>
      </div>
    <?php endforeach; ?>
  </div>
</section>
            ...
            <?php foreach ($rakhiAffiliateProducts as $prod): ?>
              <a href="<?= htmlspecialchars($prod['affiliate_url'] ?? '#') ?>" target="_blank" class="bg-[#100d10] p-3 rounded-2xl border border-[#4d444b]/40 hover:border-[#eac34a] transition-all group flex flex-col justify-between space-y-2 text-left">
                ...
                <div class="space-y-1">
                  ...
                  <span class="text-xs font-extrabold text-white block pt-1"><?= htmlspecialchars($prod['price_text'] ?? '') ?></span>
                </div>
                <div class="pt-1">
...
<section class="max-w-5xl mx-auto px-4 py-12 relative z-10 space-y-8">
  ...
  <div class="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
    <?php foreach ($media as $m): 
      ...
      $capText = htmlspecialchars($m['caption'] ?? 'Childhood Memory');
    ?>
