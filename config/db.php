        ...
        try {
            ...
            try {
                ...
                if ($chkRoyal->fetchColumn() == 0) {
                    ...
                    $pdo->exec("UPDATE templates SET active = 1, sort_order = 2, demo_url = 'https://digitalyogi24.com/gift/manvi-rakhi-v2' WHERE template_id = 'raksha_bandhan_royal'");
                }
            ...
            try {
                ...
                if ($chk5->fetchColumn() == 0) {
                    ...
                    $chkContent05->execute();
                    if ($chkContent05->fetchColumn() == 0) {
                ...
                $chkContentV2 = $pdo->prepare("SELECT COUNT(*) FROM page_content WHERE page_id = 'page_demo_rakhi_v2'");
                $chkContentV2->execute();
                if ($chkContentV2->fetchColumn() == 0) {
                    $pdo->exec("INSERT INTO page_content (page_id, partner_name, buyer_name, hint_question, hint_answer_hash, tagline_quote, favorite_singers, bg_music_url, song_title, song_artist, love_note_text, tokens_json, receiver_photo) VALUES ('page_demo_rakhi_v2', 'Manvi', 'Rajendra', 'What is our special Rakhi secret word? (Hint: RAKHI)', '$passHashRakhiV2', 'World\'s Best Sister 👑', 'Kishore Kumar', 'https://youtube.com/shorts/C-zaRcKXEP0', 'Phoolon Ka Taron Ka', 'Raksha Bandhan Special', 'Manvi Didi, mera saara pyaar aur dher saare aashirwaad iss lifafe mein h! 🧧 (Aur haan, TV remote mera hi रहेगा! 😄)', '$shagunTokensJsonV2', '{$demoPhotos[2]}')");
                }
