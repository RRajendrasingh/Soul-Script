  ...
  <header class="sticky top-0 z-50 bg-[#151215]/90 backdrop-blur-xl border-b border-[#4d444b]/30 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
    <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8 font-sans text-xs uppercase tracking-[0.15em] font-semibold">
        <a href="<?php echo APP_URL; ?>" class="text-[#d0c3cb] hover:text-[#e4b9df] transition-colors py-1">Home</a>
        <a href="<?php echo APP_URL; ?>/#gallery" class="text-[#d0c3cb] hover:text-[#e4b9df] transition-colors py-1">Templates &amp; Pricing</a>
        <a href="<?php echo APP_URL; ?>/edit.php" class="text-[#eac34a] hover:text-[#ffe088] flex items-center gap-1.5 transition-all py-1">
          ...
          if (data.results && data.results.length > 0) {
            resultsContainer.innerHTML = data.results.map(item => `
              <div class="p-2.5 bg-[#151215] hover:bg-[#221f21] rounded-xl flex items-center justify-between border border-[#4d444b]/50 transition-all">
                ...
                <button type="button" onclick="selectItunesTrack('${escapeHtml(item.previewUrl)}', '${escapeHtml(item.trackName)}', '${escapeHtml(item.artistName)}', '${escapeHtml(item.artworkUrl100)}')" class="px-3 py-1.5 bg-[#3b1e3b] text-[#eac34a] hover:bg-[#eac34a] hover:text-[#241a00] font-bold text-[11px] rounded-lg border border-[#eac34a]/40 shrink-0 transition-all">
                  + Select
                </button>
              </div>
            `).join('');
